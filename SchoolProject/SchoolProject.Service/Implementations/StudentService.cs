...
namespace SchoolProject.Service.Implementations
{
    public class StudentService : IStudentService
    {
        ...
        public async Task<string> CreateStudentAsync(Student student)
        {
            ...
            try
            {
                // Perform operations related to creating a student (e.g., inserting into the database)
            ...
            {
                // If there is an error, roll back the transaction to ensure no partial changes are committed
        ...
        public async Task<Student> GetStudentWithIncludingByIDAsync(int id)
        {
            var student = await _studentRepository.GetTableNoTracking()
                                            .Include(s => s.Department) // Include Department for specific student
                                            .Where(s => s.StudID.Equals(id)) // to whom student 
                                            .FirstOrDefaultAsync();
